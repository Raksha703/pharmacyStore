...
con.prepareStatement("UPDATE product SET "
	...
	public int getQuantityOfProduct(Product product) {
		try (Connection con = 
				DbConnection.getDatabaseConnection()){
							...
							PreparedStatement pst = 
				con.prepareStatement("select quantity from porduct where productid=?");
	...
	@Override
	public void updateQuantityAfterCancellation(Product product) {
		
		try(Connection con = DbConnection.getDatabaseConnection()){
			PreparedStatement pst = 
					con.prepareStatement("UPDATE product SET quantity = ? where productid = ?");
		
			pst.setInt(1, product.getQuantity()+1);
